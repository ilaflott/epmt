...
release6-demo-web: release/$(EPMT_FULL_RELEASE)
...
# Start Documentation(localhost:8050), Dash(localhost:8080) and Jupyter(localhost:8888)
	OS_TARGET=centos-6
	docker build -f Dockerfiles/Dockerfile.$(OS_TARGET)-epmt-test-release -t $(OS_TARGET)-epmt-test-release:$(EPMT_VERSION) --build-arg epmt_version=$(EPMT_VERSION) --build-arg install_path=/opt/minimalmetrics --build-arg epmt_full_release=release/$(EPMT_FULL_RELEASE) .
	docker run --name $(OS_TARGET)-epmt-$(EPMT_VERSION)-test-release -it --network epmt_default -p 8080:8080 -p 8050:8050 -p 8888:8888 -w /opt/minimalmetrics/epmt-$(EPMT_VERSION)/epmt-install/epmt/ --rm -h ernie $(OS_TARGET)-epmt-test-release:$(EPMT_VERSION) bash -c './epmt unittest'
