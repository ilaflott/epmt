...
def csv_has_header(f):
    ...
    Returns
    -------
    True if file has a header, False otherwise

    Notes
    -----
    We assume the header row has field names in it. So,
    this method only works with existing CSV files, which
    have pre-determined header names.
    If `f` is a file-handle it's position will be restored
    to the beginning of the file at the end of this function.
    '''
    ...
    return (s == 'tags,hostname,exename,path,args,exitcode')

def csv_format(f):
    '''
    Returns CSV format type

    Parameters
    ----------
           f : string or file-handle of the CSV file
               If f is a file handle, it must be positioned at the beginning
               of the file.
    ...
    Notes
    -----
    At present we support two CSV formats:
      '1.0' - papiex CSV file with header. Here each row
              corresponds to a thread
      '2.0' - Processed CSV file generated by processing
              a '1.0' CSV and combining threads of a process.
              This format is suitable for direct ingestion
              via the PostgreSQL COPY command
    If `f` is a file-handle, it's original position will be restored
    by the function
    '''
    return '1.0' if csv_has_header(f) else '2.0'
