...
def conv_csv_for_dbcopy(infile, outfile, jobid = ''):
    ...
    with a warning.
    The input and output paths must be distinct.
    '''
    logger = getLogger(__name__)  # you can use other name

    if infile == outfile:
        raise ValueError('The input and output CSV paths must not be identical')
...
def convert_csv_in_tar(in_tar, out_tar):
    ...
    import tarfile
    flags = None
    if (in_tar.endswith("tar.gz") or in_tar.endswith("tgz")):
        flags = "r:gz"
    elif (in_tar.endswith("tar")):
        flags = "r:"
    try:
        tar = tarfile.open(in_tar, flags)
    ...
    for name in tar.getnames():
        ...
        try:
            ...
            logger.error('ERROR: getting info for %s in %s', name, in_tar)
            return False
        logger.info('%s is %d bytes in archive' % (info.name, info.size))
        in_csv = tar.extractfile(info)
