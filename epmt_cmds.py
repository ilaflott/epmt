...
def epmt_dbsize(findwhat=['database','table','index','tablespace'], usejson=False, usebytes=False, db='EPMT'):
    from orm import get_db_size
    return(get_db_size(findwhat,usejson,usebytes,db))
...
def epmt_entrypoint(args, help):
    ...
    if args.epmt_cmd == 'dbsize':
        return(epmt_dbsize(findwhat=args.epmt_cmd_args, usejson=args.json, usebytes=args.bytes, db=args.db[0]) == False)
