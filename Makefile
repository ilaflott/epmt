...
check-release release-test-docker: $(EPMT_FULL_RELEASE)
	...
	$(DOCKER_RUN) --name $(OS_TARGET)-epmt-$(EPMT_VERSION)-test-release --network epmt-test-net --privileged $(DOCKER_RUN_OPTS) -h slurmctl $(OS_TARGET)-epmt-test-release:$(EPMT_VERSION)
...
epmt-develop develop: $(EPMT_FULL_RELEASE)
	@echo "(check-release release-test-docker) whoami: $(shell whoami)"
	$(DOCKER_BUILD) Dockerfiles/Dockerfile.$(OS_TARGET)-epmt-develop -t $(OS_TARGET)-epmt-develop:$(EPMT_VERSION) --build-arg epmt_version=$(EPMT_VERSION) --build-arg install_path=/opt/minimalmetrics --build-arg epmt_full_release=$(EPMT_FULL_RELEASE) --build-arg epmt_python_full_release=$(EPMT_PYTHON_FULL_RELEASE)  --build-arg python_version=$(PYTHON_VERSION) .
	@echo
	@echo "looking for postgres-test and epmt-test-net docker networks"
	if docker ps | grep postgres-test > /dev/null; then docker stop postgres-test; fi
	if docker network ls | grep epmt-test-net > /dev/null; then docker network rm epmt-test-net; fi
	...
	$(DOCKER_RUN) -d --rm --name postgres-test --network epmt-test-net -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=example -e POSTGRES_DB=EPMT-TEST postgres:latest
	@echo
	@echo "running epmt-develop container"
	$(DOCKER_RUN) --name $(OS_TARGET)-epmt-$(EPMT_VERSION)-develop --network epmt-test-net --privileged $(DOCKER_RUN_OPTS) -h slurmctl $(OS_TARGET)-epmt-develop:$(EPMT_VERSION)
#	#bash -c 'echo 2 > /proc/sys/kernel/perf_event_paranoid; \
#	#install_prefix=`epmt -h| grep install_prefix|cut -f2 -d:|sed 's/papiex-epmt-install/epmt-install/'`; \
#	#ls $${install_prefix}preset_settings/settings_test_pg_container.py; \
#	#cp -fv $${install_prefix}preset_settings/settings_test_pg_container.py $${install_prefix}epmt/settings.py; \
...
docker-clean:
	...
	- docker image rm --force $(OS_TARGET)-epmt-test-release:$(EPMT_VERSION) $(OS_TARGET)-epmt-build:$(EPMT_VERSION) $(OS_TARGET)-epmt-develop:$(EPMT_VERSION)
