...
check-python-native:
	...
	PAPIEX_OUTPUT=$(PWD)/ python ./epmt_cmds.py -d dump 	# Verify
	@python -V
	@echo "Tests pass!"
...
# just usage
	docker run -ti --rm -v `pwd`:/app -w /app lovato/python-2.6.6 python ./epmt_cmds.py
# start
	docker run -ti --rm -v `pwd`:/app -w /app -e PAPIEX_OUTPUT=/app/ lovato/python-2.6.6 python ./epmt_cmds.py -d start
# stop
	docker run -ti --rm -v `pwd`:/app -w /app -e PAPIEX_OUTPUT=/app/ lovato/python-2.6.6 python ./epmt_cmds.py -d stop
# dump
	docker run -ti --rm -v `pwd`:/app -w /app -e PAPIEX_OUTPUT=/app/ lovato/python-2.6.6 python ./epmt_cmds.py -d dump
# version
