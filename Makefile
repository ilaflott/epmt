...
#	@echo
##	$(DOCKER_RUN) $(DOCKER_RUN_OPTS) -it --rm --volume=$(shell pwd):$(shell pwd) -w $(shell pwd) $(OS_TARGET)-epmt-build make OS_TARGET=$(OS_TARGET) dist-test
...
$(EPMT_RELEASE) dist: $(PAPIEX_RELEASE)
	@echo "whoami: $(shell whoami)"
	@echo " - pyinstaller step"
...
test-$(EPMT_RELEASE) dist-test:
	@echo "whoami: $(shell whoami)"
	@echo " - making EPMT_RELEASE tarball and cleaning up"
#       #final location of tarfile
	if [ ! -d ./epmt-install-tests ]; then mkdir ./epmt-install-tests; fi
...
##	$(DOCKER_RUN) $(DOCKER_RUN_OPTS) --volume=$(shell pwd):$(shell pwd) -w $(shell pwd) $(OS_TARGET)-epmt-build make OS_TARGET=$(OS_TARGET) distclean dist dist-test
#=======
docker-dist: 
...
#	$(DOCKER_RUN) $(DOCKER_RUN_OPTS) --volume $(shell pwd):$(shell pwd)  -w $(shell pwd) $(OS_TARGET)-epmt-build make OS_TARGET=$(OS_TARGET) distclean dist dist-test
	$(DOCKER_RUN) $(DOCKER_RUN_OPTS) --volume $(shell pwd):$(shell pwd) --volume $(shell dirname $(PWD)):$(shell dirname $(PWD)) -w $(shell pwd) $(OS_TARGET)-epmt-build make OS_TARGET=$(OS_TARGET) dist dist-test
#	$(DOCKER_RUN) $(DOCKER_RUN_OPTS) --volume $(shell pwd):$(shell pwd)  -w $(shell pwd) $(OS_TARGET)-epmt-build make OS_TARGET=$(OS_TARGET) dist 
...
##	@env -i TERM=ansi PATH=${shell pwd}:${PATH} epmt unittest
###@env -i TERM=ansi PATH=${shell pwd}:${PATH} python3 -m unittest -v -f test.test_lib test.test_stat test.test_settings test.test_anysh test.test_submit test.test_run test.test_cmds test.test_query test.test_explore test.test_outliers test.test_db_schema test.test_db_migration
##check-integration-tests:
##	@env -i TERM=ansi PATH=${shell pwd}:${PATH} epmt integration
#=======
docker-clean:
