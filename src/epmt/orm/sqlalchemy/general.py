...
from sqlalchemy import engine_from_config, text, inspect, MetaData, Table, Column, String, Integer, ForeignKey, Boolean, DateTime, Text, Float, BigInteger, Index, UniqueConstraint, desc, asc, JSON
...
def setup_db(settings, drop=False, create=True):
    ...
    # sqlalchemy/orm/scoping.py:107: SAWarning: At least one scoped session is
    # already present.  configure() can not affect sessions that have already
    # been created.  "At least one scoped session is already present. "
...
def orm_procs_col(procs):
    ...
    if type(procs) in [list, set]:
        ...
        procs = [p['id'] if isinstance(p, dict) else p for p in procs]
...
def orm_jobs_col(jobs):
    ...
    if type(jobs) in [list, set]:
        ...
        jobs = [j['jobid'] if isinstance(j, dict) else j for j in jobs]
...
def orm_get_jobs(qs, tags, fltr, order, limit, offset, when, before, after,
                 hosts, annotations, analyses, exact_tag_only, processed=None):
